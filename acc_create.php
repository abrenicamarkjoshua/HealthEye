...
echo"<li>User Type is empty.</li>";
 ...
 $querycheck= mysql_query("select * from account where acc_type='Chairperson'");
 $row1 = mysql_fetch_assoc($querycheck);
 if($row1 != 1) {
 $query= mysql_query("select * from account where acc_email='$email'");
...
mysql_query("INSERT INTO account(acc_firstname, acc_middlename, acc_lastname, acc_username,acc_email, acc_password, acc_type
